import numpy as np
from metric_voting import SMRD
...
def test_single_winner(expanding_fp_tie_profile):
    ...
    for i in range(samples):
        winners[i] = SMRD().elect(expanding_fp_tie_profile, 1)[0]
...
def test_deterministic_multiwinner(expanding_fp_tie_profile):
    ...
    for i in range(samples):
        two_winners[i,:] = SMRD().elect(expanding_fp_tie_profile, 2)
        three_winners[i,:] = SMRD().elect(expanding_fp_tie_profile, 3)
...
def test_simple_multiwinner(simple_multiwinner_rd_profile):
    ...
    two_winners = np.zeros((samples, 2))
    for i in range(samples):
        two_winners[i,:] = SMRD().elect(simple_multiwinner_rd_profile, 2)
...
def test_next_available(two_winner_rd_profile):
    ...
    for i in range(samples):
        winners[i] = SMRD().elect(two_winner_rd_profile, 2)
