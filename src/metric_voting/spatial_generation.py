...
class Spatial:
    ...
    def generate(self, n, m):
        ...
        candidate_positions = np.array(
            [self.candidate_dist(**self.candidate_params) for _ in range(m)]
        )
        voter_positions = np.array(
            [self.voter_dist(**self.voter_params) for _ in range(n)]
        )
        ...
        for i in range(n):
            distances = [
                self.distance(voter_positions[i, :], candidate_positions[j, :])
                for j in range(m)
            ]
...
class GroupSpatial:
    ...
    def generate(self, n, m, voter_size_dist, candidate_size_dist, exact=True):
        ...
        candidate_positions = [
            [
                self.candidate_dists[i](**self.candidate_params[i])
                for _ in range(candidate_sizes[i])
            ]
            for i in range(self.candidate_groups)
            if candidate_sizes[i] != 0
        ]
        candidate_positions = np.vstack(candidate_positions)

        voter_positions = [
            [self.voter_dists[i](**self.voter_params[i]) for _ in range(voter_sizes[i])]
            for i in range(self.voter_groups)
            if voter_sizes[i] != 0
        ]
